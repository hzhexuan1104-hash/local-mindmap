...
- 左侧常驻资源/工具/扩展/设置竖栏已删除，画布默认直接使用可用左侧空间。
- 模板、查找替换、大纲、节点类型管理与设置统一为互斥的按需左侧工作面板；Esc 可关闭。
- 新增“节点”一级菜单，承载节点创建、结构操作、节点类型管理及新建子节点默认类型。
- 右侧检查器仅保留样式与备注；信息 Tab 和样式区中的类型切换/全局类型操作已移除。

### 发布前菜单层级优化

- 顶部菜单改为数据驱动的最多三级结构；文件、插件一级只保留功能类别。
- 最近文件收纳到“文件 → 打开 → 最近文件”，最多显示 5 项；动态插件命令收纳到“插件 → 插件命令 → 插件名称”。
- 多级菜单支持 hover 延迟、点击、方向键、Home/End、Esc 和 Tab 关闭，并在接近窗口边缘时向左或向上翻转。

### 发布前顶部与节点密度优化

- 顶部一级菜单收敛为“文件、编辑、节点、视图、插件、帮助”；原“插入”中的节点创建入口已合并到“节点 → 新建”。
- 顶部右侧撤销、重做、保存按钮已移除；文件状态继续相对窗口真实居中，业务入口、快捷键和 Ctrl+K command id 保持不变。
- 节点尺寸改为由文本、字号、粗细、图标和形状共同计算，并按内容/字体缓存测量；普通节点范围为 88–340px，长文本自动换行，菱形保留额外安全宽度。
- 布局、连线、小地图、视口裁剪与导出均直接使用动态节点 bounds；菱形连线保持连接左右顶点。

- 已新增独立命令注册中心、内置命令表、插件命令适配、统一搜索评分、范围前缀、最近命令、收藏与本地设置。
- Ctrl/Meta+K 和 Ctrl/Meta+Shift+P 可打开命令面板；支持键盘导航、IME 保护、异步防重复、焦点循环与关闭后焦点恢复。
- 动态结果接入现有运行时树索引、`locateNode`、最近文件、模板、节点类型与插件菜单贡献；节点查询按 100ms debounce 执行，空查询不枚举全部节点，最多渲染 50 项。
- 插件命令继续进入既有 Script / Workflow / external runner，保留 trusted、permission、action 校验、风险确认、插件日志与批量 undo 边界。
- 命令面板配置保存到 `config/command-palette-settings.json`；只持久化稳定 command id、时间与计数，不保存查询、节点正文、备注、动态参数或绝对路径。
- `.lmind` 与 `.lmplugin` 格式均未修改，功能保持纯本地离线。
- 验证结果：`npm run build` 通过；`npm run test` 通过（47 个测试文件、420 个测试）；Rust `cargo test` 通过（61 个测试）；独立性能脚本 4 个规模全部通过；`npm run tauri:dev` 已成功启动并主动停止。

上一阶段：v1.16.0 大型导图性能与导航增强 MVP

- 已增加运行时树索引、大纲窗口化、分支聚焦、层级折叠、统一定位、视口裁剪、小地图和本地性能信息。
- 性能状态不写入 `.lmind`，诊断复制不包含节点正文、备注或绝对路径。
- 当前布局仍为完整可见子树布局；导出完整图会临时关闭裁剪，后续可补充取消和临时展开全部。
...
- 以用户提供的参考 UI 图为视觉方向，借鉴暖白画布、轻面板、弱边框、柔和阴影、宽松留白和克制蓝色强调，不复制图中内容。
- 已建立全局视觉变量：应用 / 画布背景、三层表面、边框、三级文本、强调色、危险色、三档阴影、四档圆角、控件高度和左右面板宽度。
- 顶部菜单栏已精修为桌面应用式标题栏，产品信息、当前导图、文字菜单与撤销 / 重做 / 导出入口层级更清晰。
- 左侧资源区已精修为纯图标窄栏；搜索、当前导图、模板筛选和轻量模板卡片在首屏优先展示。
- 右侧检查器已精修分段 Tabs、主题 / 节点 / 类型分组、轻量信息行和备注编辑 / 预览区域。
- 画布已改为极浅点阵与柔和纸感；中心主题、普通节点、连线、选择、多选、框选、拖拽和放置目标反馈统一。
- 顶部下拉菜单、右键菜单、悬浮控制条、按钮、输入框、下拉框与弹窗视觉已统一。
- 本批未新增业务功能、数据字段或 npm 依赖；`.lmind`、节点类型包、模板包与 v1.4.1 Windows 桌面端代码均保持不变。
- 本地浏览器已验证菜单 Esc 关闭、备注 Tab、右键菜单、画布平移、Shift 框选、root position 拖拽和无文字误选。
- 验证结果：`npm run build` 通过；`npm run test` 通过（17 个测试文件、142 个测试）。

## v1.5 第一批：画布优先 UI 基础结构

- 顶部导航已重构为“文件 / 编辑 / 插入 / 视图 / 导入导出 / 更多”六组菜单。
- 文件、节点编辑、视图、导入导出、共享包、插件、性能测试和快捷键入口已按类别收纳；顶部只保留撤销、重做和导出高频操作。
- 左侧已重构为 52px 资源窄栏与约 240px 可收起面板，默认显示文件 / 模板，支持切换节点类型、查找、插件和设置。
- 右侧已重构为约 288px 可收起检查器，使用“样式 / 备注 / 信息”Tabs；备注编辑、实时预览、纯预览和放大预览保持可用。
- 画布控制已移动到右下角；左右面板收起时画布自动扩展。1280px 本地验证视口中，双侧收起后画布宽度由约 700px 增至约 1186px。
- 本批未新增业务功能，未修改 `.lmind`、节点类型包或模板包结构，未修改 Tauri / Windows 桌面端代码。
- 验证结果：`npm run build` 通过；`npm run test` 通过（17 个测试文件、142 个测试）。
