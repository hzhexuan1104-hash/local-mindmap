...
<<<<<<< HEAD
一个纯本地运行的思维导图 Web 工具，使用 React + TypeScript + Vite 构建。项目目标是提供离线可用、文件可迁移、无云端依赖的思维导图编辑体验。当前版本：v1.2.0 发布准备版。
=======
一个纯本地运行的思维导图 Web 工具，使用 React + TypeScript + Vite 构建。项目目标是提供离线可用、文件可迁移、无云端依赖的思维导图编辑体验。当前开发版本：v1.1.2。
>>>>>>> main
...
- v1.2 第二批功能：画布空白区域框选节点、Shift 框选追加选择、Ctrl+A 全选节点、Esc 统一关闭 / 清空选择、快捷键帮助弹窗完善。
- v1.2 第三批功能：拖拽调整父子层级基础版、拖拽目标高亮、root 移动保护、禁止移动到自己或子孙节点下。
=======
- v1.1.2 UI 小修：优化右侧备注面板顶部布局，折叠按钮独立到标题行，备注操作按钮单独成行，降低拥挤感。
>>>>>>> main
...
<<<<<<< HEAD
- v1.2.0 已补齐节点复制 / 剪切 / 粘贴、框选节点、Ctrl+A 全选、快捷键帮助完善和拖拽调整父子层级基础版；兄弟节点排序和复杂智能排版仍属于后续任务。
=======
- v1.1.2 已完成右侧备注面板折叠按钮布局小修，未改变 `.lmind` 数据结构。
...
- v1.2：框选矩形计算、节点命中测试、zoom / pan 坐标换算、Shift 框选追加、Ctrl+A 和 Esc 快捷键行为。
- v1.2：树操作纯函数、父节点查找、子孙判断、移动为子节点、root 保护、重复 ID 和非法结构校验。
