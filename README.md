...
- 已移除左侧常驻竖向导航栏；模板库、查找替换、大纲导航、节点类型管理和设置均从顶部菜单或 `Ctrl+K` 打开。
- 顶部菜单顺序为：文件、编辑、插入、节点、视图、插件、帮助。
- 右侧属性栏仅保留“样式”和“备注”；样式只修改当前节点视觉样式，节点类型管理位于“节点”菜单。
...
> Windows 可能缓存旧的任务栏或快捷方式图标。安装新包后，如仍显示旧图标，请删除旧快捷方式后由新安装包重新创建；对固定任务栏图标，可先取消固定再重新固定。

- v1.17.0：新增 Ctrl/Meta+K 命令面板，统一搜索内置命令、节点、最近文件、模板、节点类型和已启用插件命令；支持 `>`、`@`、`#`、`:` 范围前缀、收藏、最近命令、键盘导航、焦点恢复和本地设置。命令面板继续复用文件、节点、模板与插件的原执行入口，不绕过未保存确认、危险操作确认、runner、trusted、权限或 action 校验；历史不记录查询文本、节点正文、备注或绝对路径。

- v1.16.0：大型导图 MVP——运行时索引、大纲导航（窗口化）、分支聚焦、层级折叠、视口裁剪、小地图和本地性能诊断。裁剪与聚焦只改变视图，不会删除节点或改变 `.lmind` 树；完整图片导出会临时恢复完整渲染。
